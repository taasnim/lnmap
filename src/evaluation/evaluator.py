...
class Evaluator(object):
    ...
    def dist_mean_cosine(self, to_log, src_to_tgt):
        ...
        if src_to_tgt: 
            ...
            if self.params.norm_adv:
                p = self.encoder_A(self.src_emb.weight.data).data
                mean = p.data.mean(0, keepdim=True)
                src_emb = self.mapping_G(p.data.sub(mean.expand_as(p.data)).data).data
                q = self.encoder_B(self.tgt_emb.weight.data).data
                mean = q.data.mean(0, keepdim=True)
                tgt_emb = q.data.sub(mean.expand_as(q.data)).data
            else:
            ...
            if self.params.norm_adv:
                p = self.encoder_A(self.src_emb.weight.data).data
                mean = p.data.mean(0, keepdim=True)
                src_emb = p.data.sub(mean.expand_as(p.data)).data
                q = self.encoder_B(self.tgt_emb.weight.data).data
                mean = q.data.mean(0, keepdim=True)
                tgt_emb = self.mapping_F(q.data.sub(mean.expand_as(q.data)).data).data
            else:
    ...
    def model_selection_criterion_single(self, to_log):
        """
        Run Mean-cosine model selection criterion for A->B 
        """
        self.dist_mean_cosine(to_log, src_to_tgt=True)
              
        
    def word_translation(self, to_log, src_to_tgt):
        """
        ...
        if src_to_tgt:
            #A->B
            print(self.src_dico.lang, '-', self.tgt_dico.lang, ' word translation evaluation')
            src_emb = self.mapping_G(self.encoder_A(self.src_emb.weight.data)).data
            tgt_emb = self.encoder_B(self.tgt_emb.weight.data).data
            ...
            #B->A
            print(self.tgt_dico.lang, '-', self.src_dico.lang, ' word translation evaluation')
            src_emb = self.encoder_A(self.src_emb.weight.data).data
            tgt_emb = self.mapping_F(self.encoder_B(self.tgt_emb.weight.data)).data
    ...
    def eval_AB(self, to_log, mc=True):
        self.word_translation(to_log, src_to_tgt=True)
        if mc:
            self.dist_mean_cosine(to_log, src_to_tgt=True)
    
    def eval_BA(self, to_log, mc=True):
        self.word_translation(to_log, src_to_tgt=False)
        if mc:
            self.dist_mean_cosine(to_log, src_to_tgt=False)
