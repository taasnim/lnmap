...
def normalize_embeddings_2(emb, types, mean=None):
    """
    Normalize embeddings by their norms / recenter them.
    """
    for t in types.split(','):
...
def save_embeddings(embeddings, params, is_src, is_src2tgt):
    if is_src2tgt:
        file_name = params.src_lang + '-' + params.tgt_lang
    else:
        file_name = params.tgt_lang + '-' + params.src_lang
    if is_src:
        file_name = file_name + "_" + params.src_lang + '.vec'
        words = list(params.src_dico.id2word.values())
    else:
        file_name = file_name + "_" + params.tgt_lang + '.vec'
        words = list(params.tgt_dico.id2word.values())
    #print(file_name)
    file_name = os.path.join(params.save_emb_path, file_name)
    print("**Saving ", file_name)
    file = open(file_name, mode='w', encoding='utf-8', errors='surrogateescape')
    print('%d %d' % embeddings.shape, file=file)
    for i in range(len(words)):
        print(words[i] + ' ' + ' '.join(['%.6g' % x for x in embeddings[i]]), file=file)
    file.close()